...
struct LongBreakView: View {
    ...
    var body: some View {
        ...
        ZStack{
            VStack{
                Spacer()
                Text(pomodoroViewModel.secondsToMinutesAndSeconds(pomodoroViewModel.longBreakTimeRemaining))
                    .font(.system(size:90))
                    .foregroundColor(Color("#003f94ff"))
                Spacer()
                HStack(alignment: .center, spacing: 50) {
                    if pomodoroViewModel.longBreakTimerMode == .running || pomodoroViewModel.longBreakTimerMode == .paused {
                    ...
                    Button(action: {
                        if pomodoroViewModel.longBreakTimerMode == .running {
                            
                            pomodoroViewModel.pauseTimers()
                            tapped.toggle()
                        } else {
                            pomodoroViewModel.startLongBreakTimer()
                            tapped.toggle()
                        }
                    }) {
                        ZStack {
                            Circle()
                                ...
                                .foregroundColor(Color("#003f94ff"))
                            Image(systemName: pomodoroViewModel.longBreakTimerMode == .running ? "pause.fill" : "play.fill")
                                .foregroundColor(.black)
                                .font(.system(size: 34))
